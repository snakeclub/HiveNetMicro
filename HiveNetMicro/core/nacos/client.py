...
class NacosClient:
    ...
    # 扩展增加命名空间的处理
    #############################
    def get_namespace_id(self, namespace_name: str, timeout=None) -> str:
        """
        获取命名空间的id

        @param {str} namespace_name - 命名空间名称
        @param {float} timeout=None - 超时时间, 单位为秒

        @returns {str} - 返回命名空间的id, 如果命名空间不存在返回None
        """
        logger.info("[get-namespace-id] namespace-name:%s, timeout:%s" % (
            namespace_name, timeout))

        # 从服务器获取
        try:
            resp = self._do_sync_req("/nacos/v1/console/namespaces", None, None, None, timeout or self.default_timeout)
            content = resp.read().decode("UTF-8")
        except HTTPError as e:
            if e.code == HTTPStatus.NOT_FOUND:
                logger.warning(
                    "[get-namespace-id] namespaces not found for namespace-name:%s" % namespace_name)
                return None
            elif e.code == HTTPStatus.CONFLICT:
                logger.error(
                    "[get-namespace-id] config being modified concurrently for namespace-name:%s" % namespace_name)
                raise NacosException('Modified concurrently')
            elif e.code == HTTPStatus.FORBIDDEN:
                logger.error("[get-namespace-id] no right for namespace-name:%s" % namespace_name)
                raise NacosException("Insufficient privilege.")
            else:
                logger.error("[get-namespace-id] error code [:%s] for namespace-name:%s" % (
                    e.code, namespace_name))
                raise
        except Exception as e:
            logger.exception("[get-namespace-id] exception %s occur" % str(e))
            raise

        # 处理获取到的清单
        if content is None:
            logger.exception("[get-namespace-id] error, content is None")
            raise NacosException("Get namespaces error: content is None.")

        resp = json.loads(content)
        if resp.get('code', None) != 200:
            logger.exception("[get-namespace-id] error, response code error: %s" % truncate(resp))

        namespace_id = None
        for item in resp.get('data', []):
            if item.get('namespaceShowName', '') == namespace_name:
                namespace_id = item['namespace']
                logger.debug("[get-namespace-id] get namespace-name[%s] id from server, content is %s" % (namespace_name, namespace_id))
                break

        return namespace_id

    def is_namespace_exists(self, namespace: str, timeout=None) -> bool:
        """
        判断命名空间是否存在

        @param {str} namespace - 命名空间id
        @param {float} timeout=None - 超时时间

        @returns {bool} - 是否存在
        """
        logger.info("[is-namespace-exists] namespace:%s, timeout:%s" % (
            namespace, timeout))
        ...
        try:
            ...
            if e.code == HTTPStatus.NOT_FOUND:
                logger.warning(
                    "[is-namespace-exists] namespaces not found for namespace:%s" % namespace)
                return None
            elif e.code == HTTPStatus.CONFLICT:
                logger.error(
                    "[is-namespace-exists] config being modified concurrently for namespace:%s" % namespace)
                raise NacosException('Modified concurrently')
            elif e.code == HTTPStatus.FORBIDDEN:
                logger.error("[is-namespace-exists] no right for namespace:%s" % namespace)
                raise NacosException("Insufficient privilege.")
            else:
                logger.error("[is-namespace-exists] error code [:%s] for namespace:%s" % (
                    e.code, namespace))
                raise
        except Exception as e:
            logger.exception("[is-namespace-exists] exception %s occur" % str(e))
            raise

        # 处理获取到的清单
        if content is None:
            logger.exception("[is-namespace-exists] error, content is None")
            raise NacosException("Is namespace exists error: content is None.")

        resp = json.loads(content)
        if resp.get('code', None) != 200:
            logger.exception("[is-namespace-exists] error, response code error: %s" % truncate(resp))
    ...
    def add_namespace(self, namespace_name: str, namespace: str = None, namespace_desc: str = None,
            timeout=None) -> bool:
        """
        创建命名空间

        @param {str} namespace_name - 命名空间名
        @param {str} namespace=None - 命名空间id, 如果不传则自动生成md5
        @param {str} namespace_desc=None - 命名空间说明
        @param {float} timeout=None - 超时时间

        @returns {bool} - 是否创建成功
        """
        logger.info("[add-namespace] namespace:%s, namespace_name:%s, namespace_desc:%s, timeout:%s" % (
            namespace, namespace_name, namespace_desc, timeout))

        params = {
            "customNamespaceId": '' if namespace is None else namespace,
            "namespaceName": namespace_name,
            "namespaceDesc": '' if namespace_desc is None else namespace_desc
        }

        try:
            resp = self._do_sync_req("/nacos/v1/console/namespaces", None, None, params,
                                     timeout or self.default_timeout, "POST")
            c = resp.read()
            logger.info("[add-namespace] add namespace, namespace:%s, namespace_name:%s, namespace_desc:%s, server response:%s" % (
                namespace, namespace_name, namespace_desc, c))
            ...
            logger.exception("[add-namespace] exception %s occur" % str(e))
            raise

    def remove_namespace(self,  namespace: str, timeout=None) ->bool:
        """
        删除指定命名空间
        ...
        """
        #
        logger.info("[remove-namespace] namespace:%s, timeout:%s" % (
            namespace, timeout))
        ...
        try:
            resp = self._do_sync_req("/nacos/v1/console/namespaces", None, None, params,
                                     timeout or self.default_timeout, "DELETE")
            c = resp.read()
            logger.info("[remove-namespace]  remove namespace, namespace:%s, server response:%s" % (
                namespace, c))
            ...
            logger.exception("[remove-namespace] exception %s occur" % str(e))
            raise

    #############################
    # 增加异步处理的支持
    #############################
    async def _do_async_req(self, url, headers=None, params=None, data=None, timeout=None, method="GET"):
        """
        执行异步请求
        """
        if self.username and self.password:
        ...
        if headers:
            all_headers.update(headers)
        logger.debug(
            "[do-async-req] url:%s, headers:%s, params:%s, data:%s, timeout:%s" % (
                url, all_headers, params, data, timeout))
        tries = 0
        while True:
            try:
                server_info = self.get_server()
                if not server_info:
                    logger.error("[do-async-req] can not get one server.")
                    raise NacosRequestException("Server is not available.")
                address, port = server_info
                server = ":".join([address, str(port)])
                server_url = server
                ...
                async with aiohttp.ClientSession(
                    ...
                    async with _session.request(
                        ...
                        data=urlencode(data).encode() if data else None, ssl=ctx
                    ) as _response:
                        resp = {
                            'status': _response.status,
                            'msg': await _response.read()
                        }
                        logger.debug("[do-async-req] info from server[%s]: %s" % (server, _response))
                        return resp
            except Exception as _err:
                logger.warning(
                    "[do-async-req] server [%s] request error: %s\n %s" % (
                        server, str(_err), traceback.format_exc()
                    )
                )

            tries += 1
            if tries >= len(self.server_list):
                logger.error("[do-async-req] %s maybe down, no server is currently available" % server)
                raise NacosRequestException("All server are not available")
            self.change_server()
            logger.warning("[do-async-req] %s maybe down, skip to next" % server)

    async def async_send_heartbeat(self, service_name, ip, port, cluster_name=None, weight=1.0, metadata=None, ephemeral=True,group_name=DEFAULT_GROUP_NAME):
        """
        异步模式的发送心跳续约函数
        """
        logger.info("[async-send-heartbeat] ip:%s, port:%s, service_name:%s, namespace:%s" % (ip, port, service_name,
                                                                                        self.namespace))
        beat_data = {
            "serviceName": service_name,
            "ip": ip,
        ...
        try:
            resp = await self._do_async_req("/nacos/v1/ns/instance/beat", None, params, None, self.default_timeout, "PUT")
            if resp['status'] == 200:
                c = resp['msg']
                logger.info("[async-send-heartbeat] ip:%s, port:%s, service_name:%s, namespace:%s, server response:%s" %
                            (ip, port, service_name, self.namespace, c))
                return json.loads(c.decode("UTF-8"))
            elif resp['status'] == HTTPStatus.FORBIDDEN:
                raise NacosException("Insufficient privilege.")
            else:
                raise NacosException("Request Error, code is %s" % resp['status'])
        except Exception as e:
            logger.exception("[async-send-heartbeat] exception %s occur" % str(e))
            raise

    async def async_publish_config(self, data_id, group, content, app_name=None, config_type=None, timeout=None):
        """
        异步模式的发布配置
        """
        if content is None:
        ...
        try:
            # 不清楚为什么同步模式把params放到data中可以成功, 但异步模式要放到params才行
            resp = await self._do_async_req("/nacos/v1/cs/configs", headers=None,
                params=params, data=None,
                timeout=timeout or self.default_timeout, method="POST")
            if resp['status'] == 200:
                c = resp['msg']
                logger.info("[async-publish] publish content, group:%s, data_id:%s, server response:%s" % (
                    group, data_id, c))
                return c == b"true"
            elif resp['status'] == HTTPStatus.FORBIDDEN:
                raise NacosException("Insufficient privilege.")
            else:
                raise NacosException("Request Error, code is %s, msg is %s" % (resp['status'], str(resp['msg'])))
        except Exception as e:
            logger.exception("[async-publish] exception %s occur" % str(e))
            raise

    async def async_get_config(self, data_id, group, timeout=None, no_snapshot=None):
        """
        异步获取配置
        """
        no_snapshot = self.no_snapshot if no_snapshot is None else no_snapshot
        data_id, group = process_common_config_params(data_id, group)
        logger.info("[async_get-config] data_id:%s, group:%s, namespace:%s, timeout:%s" % (
            data_id, group, self.namespace, timeout))
        ...
        content = read_file_str(self.failover_base, cache_key)
        if content is None:
            logger.debug("[async-get-config] failover config is not exist for %s, try to get from server" % cache_key)
        else:
            logger.debug("[async-get-config] get %s from failover directory, content is %s" % (cache_key, truncate(content)))
            return content

        # get from server
        try:
            resp = await self._do_async_req("/nacos/v1/cs/configs", None, params, None, timeout or self.default_timeout)
            if resp['status'] == 200:
                content = resp['msg'].decode("UTF-8")
            elif resp['status'] == HTTPStatus.NOT_FOUND:
                logger.warning(
                    "[async-get-config] config not found for data_id:%s, group:%s, namespace:%s, try to delete snapshot" % (
                        data_id, group, self.namespace))
                delete_file(self.snapshot_base, cache_key)
                return None
            elif resp['status'] == HTTPStatus.CONFLICT:
                logger.error(
                    "[async-get-config] config being modified concurrently for data_id:%s, group:%s, namespace:%s" % (
                        data_id, group, self.namespace))
            elif resp['status'] == HTTPStatus.FORBIDDEN:
                logger.error("[async-get-config] no right for data_id:%s, group:%s, namespace:%s" % (
                    data_id, group, self.namespace))
                raise NacosException("Insufficient privilege.")
            else:
                logger.error("[async-get-config] error code [:%s] for data_id:%s, group:%s, namespace:%s" % (
                    resp['status'], data_id, group, self.namespace))
                if no_snapshot:
                    raise
        except Exception as e:
            logger.exception("[async-get-config] exception %s occur" % str(e))
            if no_snapshot:
                raise
        ...
        if content is not None:
            logger.info(
                "[async-get-config] content from server:%s, data_id:%s, group:%s, namespace:%s, try to save snapshot" % (
                    truncate(content), data_id, group, self.namespace))
            try:
                save_file(self.snapshot_base, cache_key, content)
            except Exception as e:
                logger.exception("[async-get-config] save snapshot failed for %s, data_id:%s, group:%s, namespace:%s" % (
                    data_id, group, self.namespace, str(e)))
            return content

        logger.error("[async-get-config] get config from server failed, try snapshot, data_id:%s, group:%s, namespace:%s" % (
            data_id, group, self.namespace))
        content = read_file_str(self.snapshot_base, cache_key)
        if content is None:
            logger.warning("[async-get-config] snapshot is not exist for %s." % cache_key)
        else:
            logger.debug("[async-get-config] get %s from snapshot directory, content is %s" % (cache_key, truncate(content)))
            return content

    async def async_remove_config(self, data_id, group, timeout=None):
        data_id, group = process_common_config_params(data_id, group)
        logger.info(
            "[async-remove] data_id:%s, group:%s, namespace:%s, timeout:%s" % (data_id, group, self.namespace, timeout))
        ...
        try:
            resp = await self._do_async_req("/nacos/v1/cs/configs", None, params, None,
                                     timeout or self.default_timeout, "DELETE")
            if resp['status'] == 200:
                c = resp['msg']
                logger.info("[async-remove] remove group:%s, data_id:%s, server response:%s" % (
                    group, data_id, c))
                return c == b"true"
            elif resp['status'] == HTTPStatus.FORBIDDEN:
                logger.error(
                    "[async-remove] no right for namespace:%s, group:%s, data_id:%s" % (self.namespace, group, data_id))
                raise NacosException("Insufficient privilege.")
            else:
                logger.error("[async-remove] error code [:%s] for namespace:%s, group:%s, data_id:%s" % (
                    resp['status'], self.namespace, group, data_id))
                raise NacosException("Request Error, code is %s" % resp['status'])
        except Exception as e:
            logger.exception("[async-remove] exception %s occur" % str(e))
            raise

    async def async_add_naming_instance(self, service_name, ip, port, cluster_name=None, weight=1.0, metadata=None,
                            enable=True, healthy=True, ephemeral=True,group_name=DEFAULT_GROUP_NAME):
        """
        异步注册服务实例
        """
        logger.info("[async-add-naming-instance] ip:%s, port:%s, service_name:%s, namespace:%s" % (
            ip, port, service_name, self.namespace))
        ...
        try:
            resp = await self._do_async_req("/nacos/v1/ns/instance", None, params, None, self.default_timeout, "POST")
            if resp['status'] == 200:
                c = resp['msg']
                logger.info("[async-add-naming-instance] ip:%s, port:%s, service_name:%s, namespace:%s, server response:%s" % (
                    ip, port, service_name, self.namespace, c))
                return c == b"ok"
            elif resp['status'] == HTTPStatus.FORBIDDEN:
                raise NacosException("Insufficient privilege.")
            ...
            logger.exception("[async-add-naming-instance] exception %s occur" % str(e))
            raise

    async def async_remove_naming_instance(self, service_name, ip, port, cluster_name=None, ephemeral=True, group_name=DEFAULT_GROUP_NAME):
        """
        异步取消注册服务实例
        """
        logger.info("[async-remove-naming-instance] ip:%s, port:%s, service_name:%s, namespace:%s" % (
            ip, port, service_name, self.namespace))
        ...
        try:
            resp = await self._do_async_req("/nacos/v1/ns/instance", None, params, None, self.default_timeout, "DELETE")
            if resp['status'] == 200:
                c = resp['msg']
                logger.info("[async-remove-naming-instance] ip:%s, port:%s, service_name:%s, namespace:%s, server response:%s" % (
                    ip, port, service_name, self.namespace, c))
                return c == b"ok"
            elif resp['status'] == HTTPStatus.FORBIDDEN:
                raise NacosException("Insufficient privilege.")
            ...
            logger.exception("[async-remove-naming-instance] exception %s occur" % str(e))
            raise

    async def async_list_naming_instance(self, service_name, clusters=None, namespace_id=None, group_name=None, healthy_only=False):
        """
        异步获取服务实例清单
        :param service_name:        服务名
        :param clusters:            集群名称            字符串，多个集群用逗号分隔
        ...
        :param healthy_only:         是否只返回健康实例   否，默认为false
        """
        logger.info("[async-list-naming-instance] service_name:%s, namespace:%s" % (service_name, self.namespace))
        ...
        try:
            resp = await self._do_async_req("/nacos/v1/ns/instance/list", None, params, None, self.default_timeout, "GET")
            if resp['status'] == 200:
                c = resp['msg']
                logger.info("[async-list-naming-instance] service_name:%s, namespace:%s, server response:%s" %
                        (service_name, self.namespace, c))
                return json.loads(c.decode("UTF-8"))
            elif resp['status'] == HTTPStatus.FORBIDDEN:
                raise NacosException("Insufficient privilege.")
            ...
            logger.exception("[async-list-naming-instance] exception %s occur" % str(e))
            raise
